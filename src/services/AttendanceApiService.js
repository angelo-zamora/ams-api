...
class AttendanceApiService {
    ...
    async clockIn(employee) {
        try {
            ...
            const response = await this._requestAttendanceApi(token, employee.USERNO);

            if (response.statusCode >= 400) {
                throw new Error(response.body?.message ||
                    response.body?.error || "Attendance API request failed");
            }
     ...
     * @private
    */
    async _requestAttendanceApi(token, userNo) {
        const url = apiHelper.buildUrl(
            `/kintai/attendance?id=${encodeURIComponent(userNo)}`
        );
